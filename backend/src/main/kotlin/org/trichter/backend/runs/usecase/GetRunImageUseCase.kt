...
@Service
class GetRunImageUseCase(
    private val runRepository: RunRepository,
    private val imageStorage: ImageStorage
) {
    @Transactional(readOnly = true)
    operator fun invoke(runId: UUID): StoredImage {
        val run = runRepository.findByIdOrThrow(runId)

        val imageRef = run.image?.into()
            ?: throw NotFoundException("Run $runId has no image")

        return imageStorage.get(imageRef)
            ?: throw NotFoundException("Image for run $runId not found")
    }
}